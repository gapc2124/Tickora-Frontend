...
export default function EventCard({ event }: EventCardProps) {
  ...
  const validImageUrl = event.image_url && !imgError;
      ...
      <div className="relative h-48 w-full overflow-hidden bg-zinc-800 flex items-center justify-center">
        {validImageUrl ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-accent to-purple-800 flex items-center justify-center">
            <span className="text-white/50 font-bold text-2xl uppercase tracking-widest">{event.titulo ? event.titulo.substring(0,2) : 'EV'}</span>
          </div>
        )}
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <div className="flex flex-col gap-2 mb-4 mt-auto">
          <div className="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400">
            ...
            <span>{event.fecha_evento ? new Date(event.fecha_evento).toLocaleDateString() : 'Fecha por definir'}</span>
          </div>
          <div className="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400">
        ...
        <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between mt-auto">
          <div className="flex flex-col">
            ...
            <span className="font-bold text-lg text-zinc-900 dark:text-white">S/ {event.precio ? Number(event.precio).toFixed(2) : '0.00'}</span>
          </div>
